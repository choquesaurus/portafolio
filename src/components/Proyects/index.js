...
export default function Proyects() {
  ...
  async function godata() {
    ...
    const { data } = await axios.get(
      "https://portafolio-functions-backend.vercel.app/api/lista"
    );
    //console.log(data)
  ...
  return (
    <div className="proyects">
      <Helmet>
        <meta
          name="description"
          content="En la seccion de proyectos encontraras mis ejemplos de prueba o experimentación y proyectos que eh realizado "
        />
      ...
      {load === true ? (
        ...
        proyects
          ...
          .map(
            (
              { _id, image, title, description, repository, goproyect },
              indice
            ) => (
              <Cardproyects
                key={_id}
